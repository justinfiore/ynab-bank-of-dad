...
- production 429 logs identify the budget by name, identify rate-limited tokens by their 1-based slot (never by secret value), and show the rolling API-call count from the last hour for every configured token; retry durations are shown as `1H`, `30M`, or `5S` without Java's `PT` prefix
