...
The QA HTTP client retries HTTP 429 responses for every request method until success by default. If a token env var is a comma-separated list, a 429 switches to the next token immediately with no sleep. After every token for that account returns 429, it waits for a usable case-insensitive `Retry-After` value (integer seconds or an HTTP date) or a `RateLimit-Reset`, `X-RateLimit-Reset`, or `X-Rate-Limit-Reset` epoch. Without a usable resume header it uses 5-second linear backoff (5, 10, 15, ... seconds). Each wait is capped at one hour; other 4xx responses fail immediately. Cleanup additionally spaces every client request by `QA_CLEANUP_PACING_MS` milliseconds. The cleanup-only default is `500`, an empty value also uses that default, and `0` disables pacing.
