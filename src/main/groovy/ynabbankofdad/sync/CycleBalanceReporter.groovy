...
class CycleBalanceReporter {
    ...
    /**
     * One child transfer POST moves money on two accounts. Attribute the outflow amount to the
     * source account and the inverse to the destination so cycle netChange stays balanced.
     */
    Map<String, Integer> netChangeByAccount(List<PlannedReconciliationIntent> intents) {
        ...
        (intents ?: []).findAll {
            ...
            int delta = netMilliunits(intent)
            net[key] = (net[key] ?: 0) + delta
            String transferDestination = transferDestinationAccountId(intent)
            if (intent.action == PlannedAction.CREATE && transferDestination &&
                transferDestination != accountId) {
                String destKey = accountKey(intent.targetChildKey, transferDestination, null)
                net[destKey] = (net[destKey] ?: 0) - delta
            }
        }
        net
    }
    ...
    private static String transferDestinationAccountId(PlannedReconciliationIntent intent) {
        Object parsed = parsePayload(intent)
        parsed instanceof Map ?
            parsed[DesiredMirrorFactory.TRANSFER_DESTINATION_ACCOUNT_FIELD] as String :
            null
    }
