...
- [ ] 1.4 Add failing repository/model tests for parent payee fields, top-level deletion tombstones, deleted subtransactions, cursor-only empty deltas, and cursor-based requests that do not impose the bootstrap lookback filter.
...
- [ ] 3.2 Add constrained state tables for stable source entities, append-only source revisions, current child mirrors, and ordered reconciliation operations with pending/applied/failed outcomes.
...
- [ ] 3.6 Backfill stable source entities and active child mirrors from successful legacy mappings that contain child transaction IDs while retaining all legacy audit rows.
...
- [ ] 3.8 Add real-SQLite integration fixtures for empty migration, populated single-mirror migration, duplicate migration, failed-only history, missing child IDs, repeated initialization, and transactional rollback on migration failure.
...
- [ ] 5.8 Add focused application tests for create, update, delete, missing-child recreation, repeated delete, partial batches, child isolation, state-write failures, and process-restart retries.
...
## 7. End-To-End Integration Coverage

- [ ] 7.1 Add WireMock integration coverage proving date/amount/payee edits update one existing child transaction, preserve its memo, and reset it to cleared and unapproved.
- [ ] 7.2 Add integration coverage for same-budget account updates and cross-budget delete-then-create rerouting with exact authorization-token separation.
- [ ] 7.3 Add integration coverage for parent deletion, unapproval, mapped-to-unmapped changes, and idempotent repeated deletion.
- [ ] 7.4 Add integration coverage for ordinary-to-split, split-to-ordinary, split add/edit/remove, and unrelated split mirror preservation.
- [ ] 7.5 Add integration coverage that recreates a manually missing child mirror when a later parent edit is observed.
- [ ] 7.6 Add a three-process-cycle integration scenario where the first cycle partially applies update/delete work, the second retries only unfinished work, and the third applies new incremental work and advances the cursor.
- [ ] 7.7 Add migration-to-live-flow integration coverage showing legacy duplicates in dry-run, keeping the newest active mirror, and deleting older mirrors exactly once during a live cycle.
- [ ] 7.8 Remove or update create-only assertions that conflict with authoritative reconciliation while preserving all unrelated allowance, money-movement, and sync coverage.

## 8. Documentation And Operational Rollout

- [ ] 8.1 Update `README.md`, `QUICK_START.md`, and configuration guidance with authoritative edit/deletion semantics, child-owned memo behavior, prospective mapping behavior, and missing-child recreation.
- [ ] 8.2 Document that migration can queue destructive legacy duplicate cleanup and require operators to back up SQLite state and run `--dry-run --max-cycles 1` before the first live reconciliation.
- [ ] 8.3 Document rollback limits after remote updates/deletions and the operation-history information available for manual recovery.
- [ ] 8.4 Run `./gradlew testAll` and resolve every unit and integration failure.
- [ ] 8.5 Run `./gradlew installDist` to verify packaged entry points and runtime classpaths.
- [ ] 8.6 Run a documented one-cycle sync dry run against a non-production/test configuration and verify that planned reconciliation performs no child or SQLite writes.
