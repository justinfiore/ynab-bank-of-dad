...
The prefix (if present) SHALL be prepended and the suffix (if present) SHALL be appended to the memo value inherited from the parent transaction (or generated for money-movement plans) before the child transaction payload is constructed.
...
- **THEN** the resulting memo is exactly the prefix + suffix (trimmed if both empty)
