...
The requested change must preserve the current Java 8 + Groovy 2.4.x + Gradle 4.2.1-era toolchain and should not combine with the separate tech-stack-upgrade TODO. Justin has explicitly chosen Spock for BDD-style testing and OpenWire immediately for simulating YNAB API interactions rather than postponing HTTP coverage.
...
### Decision: Add OpenWire-backed HTTP simulation in the first testing change
The current code's most important external behavior is how it reads budgets/accounts/categories and posts bulk transactions to YNAB. Those paths should be covered in the first testing increment rather than deferred. OpenWire-backed simulation gives a way to verify request/response behavior without requiring real credentials or real YNAB side effects.
...
- [Old toolchain dependency friction] → Mitigation: pin Spock/OpenWire-compatible versions that match Groovy 2.4 and Java 8, and verify with `./gradlew test`.
...
4. Add OpenWire-based HTTP integration specs for the YNAB interaction paths.
...
Current tool-based dependency research confirms `org.spockframework:spock-core:1.3-groovy-2.4` as a compatible fit for this repo's Groovy 2.4 / Java 8 stack. The requested "OpenWire" name does not surface as a credible Java HTTP mocking library for this use case; the discoverable, maintained HTTP simulation options are WireMock or MockServer instead. Implementation will therefore proceed by preserving the user-requested immediate HTTP simulation goal but using a Java-8-compatible HTTP mock library that actually fits the repo's stack and YNAB API testing needs. The exact dependency chosen will be verified by running `./gradlew test`.
