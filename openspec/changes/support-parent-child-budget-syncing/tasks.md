...
- Follow-up requested after initial PR: ignore local `logs/` output in `.gitignore`, expand SQLite state-store coverage into full real-SQLite integration tests that exercise every table interaction against a throwaway database, modularize the syncer internals, update rollout documentation in `README.md`, `QUICK_START.md`, and `CONFIGURATION.md`, and add WireMock-backed `ParentChildBudgetSyncer` integration scenarios for live/dry-run sync orchestration.
