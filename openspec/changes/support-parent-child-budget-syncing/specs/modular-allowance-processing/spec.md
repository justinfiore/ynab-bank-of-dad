...
#### Scenario: Sync-enabled config extends the same CLI safely
- **WHEN** the operator enables parent/child budget syncing in the runtime config and runs the existing CLI entry point
- **THEN** the CLI SHALL evaluate both the current allowance workflow and the configured sync workflow according to the active settings
- **AND** `--dry-run` SHALL continue to suppress all parent-budget and child-budget posting side effects
