...
- Opt-in GitHub Actions live QA runs `qaAutomatedSmoke` only on pull requests that have the `end-to-end-qa` label, are authored by `justinfiore` or `jhorgenson`, and are not from a fork. Tokens and plan UUIDs come from repository secrets / env vars and must never be written to disk. Manual A8/B5/C8 stays local.
